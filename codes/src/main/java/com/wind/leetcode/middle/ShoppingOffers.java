...
public class ShoppingOffers {
    public int shoppingOffers(List<Integer> prices, List<List<Integer>> specials, List<Integer> needs) {
        ...
        for (int i = 0; i!= needs.size(); i++) {
            minOffer += prices.get(i) * needs.get(i);
        }
        ...
        for (int i = 0; i!= specials.size(); i++) {
            ...
            if (isValid) {
                minOffer = Math.min(minOffer, shoppingOffers(prices, specials, needs) + special.get(special.size() - 1));
            }
    ...
    public int shoppingOffersWithDp(List<Integer> prices, List<List<Integer>> specials, List<Integer> needs) {
